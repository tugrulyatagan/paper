...
\par PDR plot of the lowest SF scheme and the smart prediction schemes are shown in Figure \ref{fig:prediction_pdr}. Randomly generated network radius is set to 5000 meters and number of GWs is set to 3. Prediction model needs nodes' locations and three GWs are enough to locate nodes positions by triangulation. In Table \ref{table:prediction_pdr}, PDR values for various network radii are presented. Both smart SVM and smart DTC schemes give better PDR than lowest SF schemes when number of nodes increases. Increasing number of nodes, increases number of interference. Smart schemes  improve network performance when LoRa interference is high. Moreover, smart schemes give better results when nodes are deployed closer to the GW, since nodes have margin to increase their SFs when they are deployed closer to the GW. If a node is far away from GW, then smart schemes cannot increase the SF to avoid interference since the asigned SF is already high.

\par In Table \ref{table:prediction_accuracy}, prediction accuracy of smart SVM and smart DTC schemes are presented for various network radii and number of nodes. Prediction accuracy is not directly proportional to network PDR. Correct prediction of a interfered transmission may not increase the PDR but increases the accuracy. Mart DTC gives better PDR results than smart SVM, even overall SVM prediction accuracy is high.
