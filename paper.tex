...
\par In Figure \ref{fig:prediction_pdr}, result of proposed technique is presented. PDR plot of lowest SF scheme and machine learning prediction schemes are shown in same figure. Randomly generated network topology radius is set to 5000 meters and number of GWs is set to 3. Prediction model needs nodes location and three GWs are enough to roughly locate nodes positions by triangulation. In Table \ref{table:prediction_pdr}, PDR values for different topology radius are presented. Both prediction SF schemes give better PDR than lowest SF schemes when number of nodes increases since number of interference also increases. Prediction schemes can only improve network performance when LoRa interference is present. Also, prediction schemes give better results when nodes are deployed close to the gateway, since nodes have margin to increase their SFs. If a node is far away from the GW, then prediction schemes cannot increase the SF to avoid interference since the SF is already high. 

In Table \ref{table:prediction_accuracy}, prediction accuracy of SVM and DTC prediction schemes are presented for different network topology radius and number of nodes. Number of GW is set to 3.
